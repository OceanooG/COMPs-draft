...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        # Set up a value iteration loop
        for i in range(iterations):
            iterationValues = util.Counter()
            for s in self.mdp.getStatus():
    ...
    def computeActionFromValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # initializing a stateAction counter that will hold the q-value for each state action pair. The policy or conduct that provides the greatest expected sum of benefits is chosen.
